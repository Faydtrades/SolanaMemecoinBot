...
def raw_file_path() -> Path:
    ...
    return RAW_DIR / f"live_pump_notifications_v0_3_3_{day}.jsonl"


def log_file_path() -> Path:
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    day = datetime.now(timezone.utc).strftime("%Y%m%d")
    return LOG_DIR / f"live_collector_v0_3_3_{day}.log"
...
def ensure_schema(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE INDEX IF NOT EXISTS idx_gap_jobs_status
            ON gap_jobs(status, next_retry_at_utc);
...
def insert_event(
    ...
    observed_at: str,
    source_decoded_file: str = "LIVE_WEBSOCKET_EVENT_V0_3_3",
...
def enqueue_gap_job(
    ...
    conn.execute(
        """
        INSERT OR IGNORE INTO gap_jobs(
            gap_key,
            gap_started_at_utc,
            gap_ended_at_utc,
            before_signature,
...
def mark_gap_in_progress(
    ...
    conn.commit()
    row = conn.execute(
        "SELECT attempts FROM gap_jobs WHERE gap_key=?",
        (gap_key,),
    ).fetchone()
    ...
    async with httpx.AsyncClient(
        ...
        while True:
            ...
            confirmation_backlog = conn.execute(
                "SELECT COUNT(*) AS n FROM confirmation_jobs WHERE status != 'DONE'"
            ).fetchone()["n"]
            deep_backlog = conn.execute(
                "SELECT COUNT(*) AS n FROM deep_enrichment_jobs WHERE status != 'DONE'"
            ).fetchone()["n"]
                    ...
                    for tx_entry in block.get("transactions") or []:
                        ...
                        for event_index, payload in enumerate(payloads):
                            ...
                            inserted = insert_event(
                                ...
                                decoded,
                                observed_at,
                                source_decoded_file="GAP_RECONCILIATION_V0_3_3",
                                confirmation_status="confirmed",
            ...
            async with connect(
                ...
                while not listener_stop.is_set():
                    ...
                    append_jsonl(
                        raw_path,
                        {
                            "schema_version": "live_pump_notification_v0.3.3",
...
async def run(
    ...
    print("=" * 104)
    print("SOLANA MEMECOIN BOT - ROBUST LIVE COLLECTOR v0.3.3")
    print("=" * 104)
