...
NS4 = "tradingbot_local_phase4_b1d2_lifecycle_probe"
...
def main() -> int:
    print("=" * 116)
    print("PHASE 4.3B1D2 - LIFECYCLE API REPLAY PROBE v0.1")
    print("=" * 116)
    ...
    print("-" * 116)
    try:
        source = inspect.getsource(Store.list_open_positions)
        print(source)
    ...
    if core_ok and not checks["runtime_equals_locked_order"]:
        ...
        print(
            "The lifecycle API returns the exact three locked tracks deterministically, "
            "but in a different order than LOCKED_PHASE4_TRACKS. "
            "The original B1D2 validator incorrectly treated ordering as semantic."
        )
    ...
    if core_ok and checks["runtime_equals_locked_order"]:
        ...
        print(
            "The same persisted DB now reproduces the exact locked order through "
            "list_open_positions(). The original runtime mismatch is therefore not "
            "a persisted lifecycle error and not reproducible from current state. "
            "Inspect the original validator expression/types before closing B1D2."
        )
        print("RESULT: CHECK")
        return 2
