...
from phase3.buyer_response_block_research_v0_1 import (
    FIXED_D_RECLAIM, FIXED_D_RUNAWAY, PARETO_METRICS,
    generate_buyer_response_combinations, parameter_set_for_buyer_response,
    pareto_frontier, validate_locked_search, validate_selection,
)

HORIZONS={5000:"5s",15000:"15s",30000:"30s",60000:"60s",120000:"2m",300000:"5m"}
CHECKPOINT_SCHEMA="P3-EXP0004-C-WORK-0.1"
...
def verify_registry(path):
    ...
    if r.get("next_experiment_id")!="EXP-0004": raise RuntimeError("registry next ID must be EXP-0004")
    exp3=[x for x in r.get("experiments",[]) if isinstance(x,dict) and x.get("experiment_id")=="EXP-0003"]
    if len(exp3)!=1 or exp3[0].get("status")!="COMPLETE": raise RuntimeError("EXP-0003 not registered COMPLETE")
    return r,exp3[0]
...
def outcome_row(combo,o):
    row={
        "parameter_set_id":combo.parameter_set_id,"combo_index":combo.index,
        "ab_role":combo.ab_role,"ab_source_parameter_set_id":combo.ab_source_parameter_set_id,
        ...
        "min_rebound_bps":combo.min_rebound_bps,"min_buys":combo.min_buys,
...
def summarize_combo(combo,ext,outcomes):
    row={
        "parameter_set_id":combo.parameter_set_id,"combo_index":combo.index,
        "ab_role":combo.ab_role,"ab_source_parameter_set_id":combo.ab_source_parameter_set_id,
        "min_return_bps":combo.min_return_bps,"min_trades_since_t0":combo.min_trades_since_t0,
...
def validate_cp(payload,combo,selection_sha):
    ...
    if payload.get("selection_sha256")!=selection_sha: raise RuntimeError(f"combo {combo.index}: selection mismatch")
    if int(payload.get("combo_index",-1))!=combo.index or payload.get("parameter_set_id")!=combo.parameter_set_id:
        raise RuntimeError(f"combo {combo.index}: checkpoint identity")
    ...
    if int(s.get("run_count",-1))!=3901 or int(s.get("accounted_run_count",-2))!=3901:
        raise RuntimeError(f"combo {combo.index}: terminal accounting")
    if int(s.get("candidate_count",-1))!=len(rows):raise RuntimeError(f"combo {combo.index}: outcome row mismatch")

def main():
    ap=argparse.ArgumentParser()
    freeze_dir=PROJECT_ROOT/"data/research/phase2_research_freeze_v0_1";p3=PROJECT_ROOT/"data/research/phase3"
    ...
    ap.add_argument("--selection",default=str(p3/"EXP-0003/EXP-0003_B_selection_v0_1.json"))
    ap.add_argument("--coverage",default=str(p3/"collector_coverage_DEV-FREEZE-0001_v0_1.json"))
    ap.add_argument("--out-dir",default=str(p3/"EXP-0004"))
    ...
    print("PHASE 3 / EXP-0004 — BLOCK C BUYER RESPONSE HARNESS v0.1")
    print("="*72);started=time.time()
    registry,exp3=verify_registry(paths["registry"])
    ...
    if exp3.get("selection_sha256")!=selection_sha:raise RuntimeError("registry selection binding mismatch")
    ...
    coverage.validate_binding(freeze_id=str(dataset["freeze_id"]),
    ...
    try:
        ...
        Phase1QuoteAwareAdapterV01.validate_schema(conn);raw=Phase1QuoteAwareAdapterV01.fetch_all_bot_truth_rows(conn)
    finally:conn.close()
    ...
    out=paths["out_dir"];out.mkdir(parents=True,exist_ok=True);work=out/"_work_v0_1";work.mkdir(parents=True,exist_ok=True)
    pre=sum(cp_path(work,c.index).exists() for c in combos)
    print(f"Freeze                         : {dataset['freeze_id']}")
    print(f"Frozen events                  : {len(events)}")
    print("A+B carry-forward configs      : 4")
    print("Locked C combinations / AB     : 150")
    print(f"A+B+C combinations             : {len(combos)}")
    print("Buyer-response window          : 3s fixed")
    print("D reclaim/runaway              : fixed foundation anchors")
    ...
    for i,combo in enumerate(combos,1):
        params=parameter_set_for_buyer_response(combo);FirstPullbackStrategyV02.validate_parameters(params)
        ...
        if cp.exists():
            ...
            if ext.run_count!=3901 or ext.accounted_run_count!=3901 or ext.candidate_count!=ext.candidate_state_count:
                raise RuntimeError(f"{combo.parameter_set_id}: terminal accounting failed")
            outcomes=[replay.replay(c.reference,obs_by_mint.get(c.signal.mint,())) for c in cands]
            s=summarize_combo(combo,ext,outcomes);rows=[outcome_row(combo,o) for o in outcomes];sig=candidate_signature(cands)
            write_json_atomic(cp,{"schema_version":CHECKPOINT_SCHEMA,"experiment_id":"EXP-0004",
        ...
        if i==1 or i%10==0 or i==len(combos):
            print(
                f"[{i:3d}/600] AB={combo.ab_role:8s} RB={combo.min_rebound_bps:5d} "
                f"BU={combo.min_buys:1d} FL={combo.min_net_flow_reserve_ppm:6d} "
                f"cand={int(s['candidate_count']):3d} source={source:7s} elapsed={time.time()-started:7.1f}s"
            )

    for idx in (1,300,600):
        combo=combos[idx-1];second,_=extract_candidates(events,parameter_set_for_buyer_response(combo))
    ...
    sp=out/"EXP-0004_C_buyer_response_summary_v0_1.csv"
    fp=out/"EXP-0004_C_buyer_response_frontier_v0_1.csv"
    lp=out/"EXP-0004_C_buyer_response_candidate_outcomes_v0_1.csv"
    rp=out/"EXP-0004_C_buyer_response_report_v0_1.json"
    mp=out/"EXP-0004_manifest_v0_1.json"
    write_csv(sp,summaries);write_csv(fp,frontier);write_csv(lp,longs)

    report_core={
        "schema_version":"P3-EXP0004-C-0.1","experiment_id":"EXP-0004","block":"C_BUYER_RESPONSE_3S",
        ...
        "design":{"AB_carry_forward_count":4,"C_combination_count_per_AB":150,"combination_count":600,
                  "buyer_response_window":"3s","D_reclaim_fixed":FIXED_D_RECLAIM,"D_runaway_fixed":FIXED_D_RUNAWAY,
                  ...
                  "robustness_goal":"cross-AB and neighboring-C stability; no highest-median selection"},
        "selection_binding":{"path":str(paths["selection"].relative_to(PROJECT_ROOT)).replace("\\","/"),"sha256":selection_sha},
        ...
        "determinism":{"sentinel_indices":[1,300,600],"candidate_signatures":{str(i):sigs[i] for i in (1,300,600)},"pass":True},
    ...
    manifest_out={
        "manifest_schema_version":"EXPM-0.1-P3EXT","experiment_id":"EXP-0004",
        "status":"RUN_COMPLETE_PENDING_POSTRUN_AUDIT_AND_REVIEW",
        "hypothesis":"Buyer-response thresholds change candidate incidence and post-signal outcomes across four locked A+B regimes.",
        ...
        "decision":"REVIEW_CROSS_AB_AND_NEIGHBORING_C_STABILITY_BEFORE_BLOCK_D",
        "notes":"Development/discovery data only; no winner, no PnL, no exit optimization."
    }
    mp.write_text(json.dumps(manifest_out,indent=2,sort_keys=True)+"\n",encoding="utf-8")
    ...
    print("Single C winner selected       : NO")
    print(f"Report                         : {rp}")
