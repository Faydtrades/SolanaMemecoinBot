...
SCHEMA_VERSION = "phase4_3b1d0_runtime_price_impact_probe_v0.1"
ROW_LIMIT = 50_000
...
def load_rows(conn: sqlite3.Connection) -> list[sqlite3.Row]:
    # Latest bounded slice. Reverse after fetch so each mint is processed
    # chronologically by production rowid inside the sampled interval.
    rows = conn.execute(
        """
        SELECT
            rowid AS p1_rowid,
...
def main() -> int:
    print("=" * 122)
    print("PHASE 4.3B1D0 - RUNTIME PRICE-IMPACT MECHANICS PROBE v0.1")
    print("=" * 122)
    print(f"Project root                         : {PROJECT_ROOT}")
    print(f"Production DB                        : {PRODUCTION_DB}")
    print("Production DB mode                   : QUERY-ONLY / READ-ONLY")
    print(f"Latest trade-row bound               : {ROW_LIMIT:,}")
    ...
    print("VALIDATION")
    print("-" * 122)
    print(f"Production SQLite quick_check          : {'PASS' if str(quick).lower() == 'ok' else 'FAIL'}")
