...
def fmt_stat(path: Path) -> tuple[int, int]:
    st = path.stat()
        ...
        if role_hits and (interesting_keys & keys or len(role_hits) >= 2):
            ...
            for k, v in obj.items():
                if k in interesting_keys or k in LOCKED_ENTRY_ROLES:
                    compact[k] = v
            if not compact:
                compact = {k: v for k, v in obj.items() if isinstance(v, (str, int, float, bool, type(None)))}
...
def inspect_production_db() -> tuple[bool, dict[str, Any]]:
    if not PRODUCTION_DB.exists():
        return False, {"error": "production DB missing"}

    size_before, mtime_before = fmt_stat(PRODUCTION_DB)
        ...
        if "pump_events" in tables:
            ...
            row = conn.execute(
                ...
                SELECT COUNT(*) AS n,
                       ...
                       MAX(decoded_at_utc) AS latest_observed
                FROM pump_events
                """
            ).fetchone()
            ...
            cols, event_count, max_rowid, latest_observed = [], 0, None, None
    finally:
        conn.close()

    size_after, mtime_after = fmt_stat(PRODUCTION_DB)
    unchanged = size_before == size_after and mtime_before == mtime_after
    required_cols = {
...
def main() -> int:
    print("=" * 108)
    print("PHASE 4.3B0 - LIVE CANDIDATESIGNAL INTEGRATION PREFLIGHT v0.1")
    print("=" * 108)
    ...
    print()
    print("-" * 108)
    print("PRODUCTION DB READ-ONLY CONTRACT")
    print("-" * 108)
    db_ok, db_info = inspect_production_db()
    if "error" in db_info:
        print(f"DB probe                             : FAIL | {db_info['error']}")
    else:
        print(f"SQLite quick_check                   : {db_info['quick_check']}")
        print(f"Pump events                          : {db_info['event_count']}")
        print(f"Pump max rowid                       : {db_info['max_rowid']}")
        ...
        print(f"DB unchanged during query-only probe : {'PASS' if db_info['db_unchanged_during_readonly_probe'] else 'FAIL'}")
    ...
    print(f"Locked roles discovered              : {tuple(r for r in LOCKED_ENTRY_ROLES if r in found_roles)}")
    ...
    if not findings:
        ...
        print("This is not itself a strategy failure; the next build must bind to the exact local Phase3 artifact path shown above.")
    else:
        for i, item in enumerate(findings[:20], 1):
            print(f"[{i:02d}] source={item['source']} | roles={item['roles']}")
            print("     " + json.dumps(item["object"], ensure_ascii=False, sort_keys=True, default=str)[:1800])
    ...
    print(f"Production DB read-only contract     : {'PASS' if db_ok else 'FAIL'}")
    print(f"CONTROL + ROBUST_1/2/3 discoverable  : {'PASS' if roles_ok else 'CHECK'}")
    print("CandidateSignal remains strategy boundary: PASS (no adapter mutation in this preflight)")
    ...
    if files_ok and db_ok and roles_ok:
        print("RESULT: PASS")
        print("NEXT: build Phase 4.3B1 live CandidateSignal source + controlled live smoke against these exact interfaces.")
        return 0

    print("RESULT: CHECK")
    print("Do not start live 4.3B1 yet. Return this output so the missing local binding can be resolved without guessing.")
    return 2
