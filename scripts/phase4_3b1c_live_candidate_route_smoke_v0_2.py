...
def main() -> int:
    ...
    # B1C v0.2 PRE-FLIGHT:
    # Strategy Clock v0.2 is the compatibility clock for FirstPullback v1.1.
    # Validate all four exact locked parameter sets BEFORE collector startup.
    for _role, _params in params_by_role.items():
        if _params.strategy_version != "v1.1":
            raise RuntimeError(
                f"Unexpected locked strategy version for {_role}: "
                f"{_params.strategy_version}"
            )
        Strategy.validate_parameters(_params)

    if not hasattr(Clock, "schedule_entry_expiry") or not hasattr(Clock, "fire_if_due"):
        raise RuntimeError("Strategy Clock v0.2 runtime contract incomplete")
    ...
    print("=" * 118)
    print("PHASE 4.3B1C - LIVE CANDIDATESIGNAL -> SIMULATED ENTRY ROUTING SMOKE v0.2")
    print("=" * 118)
    ...
    print("Clock compatibility preflight      : PASS — v1.1 locked params")
    ...
    audit_payload = {
        "schema_version": "phase4_3b1c_live_candidate_route_smoke_v0.2",
