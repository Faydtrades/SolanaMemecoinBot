...
NS = "tradingbot_local_phase2_v034_live_source_smoke"
...
def stop_process(proc: subprocess.Popen, grace_seconds: int = 15) -> tuple[str, int | None]:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Phase 4.3B1B v0.3.4 controlled live source smoke."
    )
    ...
    if not COLLECTOR.exists():
        raise SystemExit(
            f"Missing v0.3.4 collector: {COLLECTOR}\n"
            "Build and self-test v0.3.4 first."
        )
    ...
    collector_log = AUDIT_DIR / f"phase4_3b1b_v034_collector_{stamp}.log"
    audit_path = AUDIT_DIR / f"phase4_3b1b_v034_live_source_smoke_{stamp}.json"
    ...
    print("=" * 114)
    print("PHASE 4.3B1B - v0.3.4 CONTROLLED LIVE SOURCE SIDECAR SMOKE v0.1")
    print("=" * 114)
    ...
    print(f"[INFO] v0.3.4 collector PID: {proc.pid}")
    print("[INFO] Sidecar will stop collector automatically.")
    print()
    ...
    try:
        while True:
            ...
            if proc.poll() is not None:
                ...
                print(
                    f"[CHECK] Collector exited early after {elapsed:.1f}s "
                    f"returncode={collector_returncode}"
                )
                break
            ...
            if rows:
                ...
                causal_rows = [
                    row for row in rows if row_mint(row) in eligible_mints
                ]
            ...
            if now >= next_health:
                print(
                    ...
                    f"rowid={cursor} "
                    f"rows={rows_seen} "
                    f"launches={launch_rows_seen} "
                    f"eligible_mints={len(eligible_mints)} "
                    f"usable={usable_events} "
                    f"states={feature_states} "
    ...
    end_cursor = cursor
    new_pump_rows = end_cursor - start_cursor
    ...
    payload = {
        "schema_version": "phase4_3b1b_v034_live_source_smoke_v0.1",
        ...
        "production_db_sidecar_access": "QUERY_ONLY_READ_ONLY",
        "start_cursor_rowid": start_cursor,
        "end_cursor_rowid": end_cursor,
    ...
    print(
        "Do not advance to CandidateSignal routing. "
        "Return this output for diagnosis."
    )
    return 2
