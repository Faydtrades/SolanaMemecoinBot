...
def main() -> None:
    ...
    print("PHASE 3 / EXP-0002 — BLOCK A IMPULSE HARNESS v0.1")
    print("=" * 68)
    started = time.time()
    ...
    for i, combo in enumerate(combos, start=1):
        ...
        if extraction.expired_count + extraction.invalidated_count + extraction.candidate_state_count != extraction.run_count:
            raise RuntimeError(f"{combo.parameter_set_id}: terminal lifecycle incomplete")

        outcomes = [
            replay.replay(c.reference, obs_by_mint.get(c.signal.mint, ()))
            for c in candidates
        ]
        candidate_signatures[combo.index] = candidate_signature(candidates)
        summary_rows.append(summarize_combo(combo, extraction, outcomes))
        long_rows.extend(outcome_long_row(combo, o) for o in outcomes)
        ...
        if i == 1 or i % 10 == 0 or i == len(combos):
            ...
            print(
                ...
                f"candidates={extraction.candidate_count:3d} elapsed={elapsed:6.1f}s"
            )
    ...
    summary_path = out_dir / "EXP-0002_A_impulse_summary_v0_1.csv"
    frontier_path = out_dir / "EXP-0002_A_impulse_frontier_v0_1.csv"
    long_path = out_dir / "EXP-0002_A_impulse_candidate_outcomes_v0_1.csv"
    report_path = out_dir / "EXP-0002_A_impulse_report_v0_1.json"
    manifest_path = out_dir / "EXP-0002_manifest_v0_1.json"
    ...
    report_core = {
        "schema_version": "P3-EXP0002-A-0.1",
    ...
    exp_manifest = {
        ...
        "status": "COMPLETE_BLOCK_A_CHARACTERIZATION_IF_RESULT_PASS",
        "hypothesis": (
        ...
        "notes": (
            ...
            "not constitute a final trading parameter selection."
        ),
    }
