"""V3 missing disjoint/protocol original-API states; SYNTHETIC_EXTERNAL_FACTS.
...
def protocol_fixture():
    ...
    path=EVIDENCE/'public_protocol_resource_v1.py'
    spec=importlib.util.spec_from_file_location('joint_protocol_resource',path)
...
def hot_plan(mode):
    if mode=='protocol_pending':return [],[]
...
def protocol_decide(f, root, ordinal, at):
    """Original typed RPC -> protocol -> Runtime -> Ledger nonacceptance.

    The extra _admit_next source page is retained and counted, never bypassed.
    Large mock wallet facts cannot change the original supported opening wallet.
    """
    fixture=protocol_fixture()
    from live.public_rpc_v0_1 import PublicReadOnlyRpc
    from live.wallet_evidence_v0_1 import WalletEvidenceAdapter,WalletEvidenceRequest
    from live.ledger_settlement_v0_1 import WalletSupportInput
    from live.pump_protocol_compatibility_v0_1 import PumpProtocolObservation
    from live.ledger_evidence_codec_v0_1 import wallet_observation_to_json
    import httpx
    started=time.perf_counter_ns();item=f.repo.candidate(root)
    scenario=owned.a3.Scenario();slot=102+ordinal
    scenario.context=scenario.multiple_context=slot;scenario.initial_slot=slot
    scenario.upper_slot=slot+1;scenario.block_time=at-20
    expected,shape=fixture.configure(scenario,item.mint,f.domain.wallet)
    request=WalletEvidenceRequest(f.domain.wallet,f.domain.genesis_hash,slot,expected)
    with PublicReadOnlyRpc('https://fixture.invalid',fixture.PROFILE,transport=httpx.MockTransport(scenario.handle)) as rpc:
        observation=WalletEvidenceAdapter(rpc,request,clock=lambda:owned.a3.utc(at)).observe()
        support=WalletSupportInput(observation,owned.a3.utc(at),slot)
        read=rpc.get_multiple_accounts((item.mint,fixture.derive_bonding_curve_pda(item.mint),
            fixture.derive_pumpswap_pool_pda(item.mint)),min_context_slot=slot)
    protocol=PumpProtocolObservation(item.mint,support.digest,f.domain.genesis_hash,f.domain.genesis_hash,
        fixture.PROFILE.fingerprint,read,observation.anchor,owned.a3.utc(at))
    before=f.source.count();clock=lambda:owned.a3.clock(f.repo,at)
    assert protocol.classify(f.domain,item,clock(),support)[0]=='UNSUPPORTED'
    result=f.runtime.step(clock=clock,entry=owned.runtime.EntryFacts('PUMP',owned.sf.TOKEN_PROGRAM_ID,support,protocol),
        source_cut_utc=owned.a3.utc(at),operations_resources=lambda:owned.fixtures.host(f,at))
    assert result.work=='CANDIDATE_UNSUPPORTED',asdict(result)
    assert f.source.count()==before+1 and f.repo.inbox_disposition(root)=='REJECTED'
    assert not f.repo.consumer_snapshot()['funding'].quarantine_reasons
    f.active_gate.after_step(result,f.started)
    assert f.active_gate.snapshot()['held_reason'] is None,f.active_gate.snapshot()
    payload=f.repo._conn.execute('SELECT payload_json FROM ledger_inbox_dispositions WHERE root_id=? ORDER BY ordinal DESC LIMIT 1',(root,)).fetchone()[0]
    persisted=json.loads(payload)
    assert persisted['protocol_evidence']['wallet_observation_json']==wallet_observation_to_json(observation)
    f.last_decision_us=(time.perf_counter_ns()-started)//1000
    return dict(result=asdict(result),wallet_json_bytes=len(wallet_observation_to_json(observation).encode()),
        persisted_disposition_bytes=len(payload.encode()),persisted_disposition_sha256=hashlib.sha256(payload.encode()).hexdigest(),
        protocol_digest=protocol.content_digest,shape=shape,source_digest=f.source.latest().content_digest,
        source_records_added=f.source.count()-before)


def protocol_candidate_source_unit(f, ordinal, at):
    """Use original Runtime queue intake, not a manually populated queue.

    The V2 direct-Authority fixture drained handoff itself. Protocol Runtime
    requires that this first page populate its original queue before the next
    page can classify the corresponding supplied public facts.
    """
    ...
    f.last_source_unit_us=(time.perf_counter_ns()-began)//1000
    return verdict,roots,f.producer.durable_p1_rowid-cursor,len(sh.canonical_json(verdict).encode())
...
def build(output, source, roots=65, views=627, incident_pairs=951, hot_mints=32, hot_events=32, late_startups=99, gated=True, mode='hottest_pending'):
    ...
    with ExitStack() as stack:
        if mode=='protocol_pending':
            # Every binding is established at original initialization; no
            # existing baseline, receipt or persisted profile is rewritten.
            import live_ledger_baseline_selftest_v0_1 as baseline_fixture
            accepted_rpc=protocol_fixture().PROFILE
            original_observe=owned.a3.observe
            stack.enter_context(patch.object(baseline_fixture,'PROFILE',accepted_rpc))
            stack.enter_context(patch.object(owned.a3,'PROFILE',accepted_rpc))
            stack.enter_context(patch.object(owned.sf,'PROFILE',accepted_rpc))
            stack.enter_context(patch.object(owned.a3,'observe',lambda *a,**kw:original_observe(*a,**dict(kw,profile=accepted_rpc))))
        ...
        try:
            ...
            protocol_units=roots-4 if mode=='protocol_pending' else 0
            idle=views-initial_count-roots-hot_units-protocol_units
            assert idle>=0
            ordinal=0
            ...
            for n in range(roots):
                ...
                at=owned.NOW+4+14*n
                if mode=='protocol_pending' and n not in (0,1,roots-2,roots-1):
                    verdict,new,raw,size=protocol_candidate_source_unit(f,ordinal,at)
                else:verdict,new,raw,size=source_unit(f,ordinal,at,1,candidate=True)
                root=f.root if n==0 else new[0]
                assert n==0 or len(new)==1,('candidate-count',n,len(new))
                admit=n in (0,1,roots-2) or (n==roots-1 and mode in ('hottest_pending','protocol_pending'))
                protocol_record=None
                if mode=='protocol_pending' and not admit:
                    protocol_record=protocol_decide(f,root,n,at)
                    action=None
                else:action=decide(f,root,n,at,admit)
                if action is not None:
                    try:
                        ...
                        with patch.object(owned,'DryPublicFacts',current_cut),patch.object(owned.external.a4,'evidence',current_evidence):
                            result,_=owned.execute(f,action,at=at+5,point='before-terminal' if n==roots-1 and mode in ('hottest_pending','protocol_pending') else None)
                ...
                raw_total+=raw
                log.append(dict(unit=ordinal,kind='ADMISSION' if admit else 'PROTOCOL_UNSUPPORTED' if protocol_record else 'VENUE_DENIAL',root=root,
                    source_digest=verdict.content_digest,bytes=size,source_unit_us=f.last_source_unit_us,
                    decision_us=f.last_decision_us,protocol_record=protocol_record))
                ordinal+=1
                if protocol_record:
                    log.append(dict(unit=ordinal,kind='ORIGINAL_RUNTIME_PROTOCOL_SOURCE_PAGE',root=root,
                        source_digest=protocol_record['source_digest'],bytes=len(sh.canonical_json(f.source.latest()).encode())))
                    ordinal+=1
            ...
            expected_events=0 if mode=='protocol_pending' else 1015 if mode=='hottest_pending' else 1024
            expected_active=0 if mode=='protocol_pending' else 32 if mode=='hottest_pending' else 64 if mode=='active64_hold' else 4
            assert metrics['RETAINED_EVENTS']==expected_events and metrics['UNFINISHED_MINTS']==expected_active,metrics
            final_gate=None
            if mode not in ('hottest_pending','protocol_pending'):
            ...
            if mode=='protocol_pending':config['rpc_profile']=asdict(protocol_fixture().PROFILE)
            custody=f.repo.consumer_snapshot()
            assert not custody['positions'] and not custody['protections'] and len(custody['reservations'])==int(mode in ('hottest_pending','protocol_pending'))
            ...
            write(output/'construction.json',dict(schema='MEME_LIVE_T010_JOINT_SYNTHETIC_STATE_V1',
                ...
                protocol_additional_source_pages=protocol_units,
                rpc_profile=asdict(protocol_fixture().PROFILE) if mode=='protocol_pending' else None,
                ...
                clock_scope='Original synthetic TrustedClockSample and original synthetic policy with one-hour checkpoint interval; not approved real-clock behavior or public freshness qualification.',
...
if __name__=='__main__':
    ...
    p.add_argument('--mode',choices=('hottest_pending','active64_hold','four_hot_hold','protocol_pending'),required=True)
    ...
    roots=a.roots or (98 if a.mode=='protocol_pending' else 73 if a.mode=='four_hot_hold' else 65 if a.mode=='hottest_pending' else 64)
    views=a.views or (626 if a.mode=='active64_hold' else 627)
    mints,events=(0,0) if a.mode=='protocol_pending' else (4,256) if a.mode=='four_hot_hold' else (32,32)
