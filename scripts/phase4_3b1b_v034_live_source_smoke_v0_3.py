...
NS = "tradingbot_local_phase2_v034_live_source_smoke_v03"
...
def fetch_after(conn: sqlite3.Connection, cursor: int, limit: int):
    # IMPORTANT v0.3 FIX:
    # Phase1ReadonlyAdapterV01's row contract expects p1_rowid.
    return conn.execute(
        """
...
def main() -> int:
    ...
    if not COLLECTOR.exists():
        raise SystemExit(f"Missing collector: {COLLECTOR}")
    ...
    collector_log = AUDIT_DIR / f"phase4_3b1b_v034_collector_v03_{stamp}.log"
    audit_path = AUDIT_DIR / f"phase4_3b1b_v034_live_source_smoke_v03_{stamp}.json"
    ...
    print("=" * 116)
    print("PHASE 4.3B1B - v0.3.4 CONTROLLED LIVE SOURCE SIDECAR SMOKE v0.3")
    print("=" * 116)
    print(f"Project root                     : {PROJECT_ROOT}")
    print(f"Collector                        : {COLLECTOR}")
    print(f"Production DB                    : {PRODUCTION_DB}")
    print("Adapter return contract          : normalize_rows -> (events, skipped)")
    print("Adapter row identity contract    : rowid AS p1_rowid")
    ...
    payload = {
        "schema_version": "phase4_3b1b_v034_live_source_smoke_v0.3",
        "started_at_utc": datetime.fromtimestamp(
            time.time() - elapsed, tz=timezone.utc
        ).isoformat(),
        "finished_at_utc": utc_now().isoformat(),
        "elapsed_seconds": round(elapsed, 3),
        "adapter_return_contract": "events_skipped_tuple",
        "adapter_rowid_contract": "p1_rowid",
    ...
    print("Adapter return contract                 : PASS")
    print("Adapter p1_rowid contract               : PASS")
    ...
    print(f"Feature none                            : {feature_none}")
