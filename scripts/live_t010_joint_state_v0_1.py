"""Original-API synthetic joint-state construction; never a public campaign.
...
def append_hot_batch(f, batch, mints, events, offset):
    with sqlite3.connect(f.raw) as c:
        last=c.execute('SELECT max(rowid) FROM pump_events').fetchone()[0]
        for i in range(32):
            ordinal=batch*32+i;event=ordinal//mints;mint_number=ordinal%mints
...
def build(output, source, roots=128, views=627, incident_pairs=951, hot_mints=0, hot_events=0, late_startups=1, gated=False):
    ...
    with ExitStack() as stack:
        ...
        try:
            ...
            hot_units=hot_mints*hot_events//32
            ...
            for n in range(roots):
                ...
                admit=n in (0,1,roots-2,roots-1)
                ...
                if action is not None:
                    try:
                        ...
                        with patch.object(owned,'DryPublicFacts',current_cut),patch.object(owned.external.a4,'evidence',current_evidence):
                            result,_=owned.execute(f,action,at=at+5,point='before-terminal' if n==roots-1 else None)
            ...
            assert not custody['positions'] and not custody['protections'] and len(custody['reservations'])==1
...
if __name__=='__main__':
    ...
    p.add_argument('--roots',type=int,default=128);p.add_argument('--views',type=int,default=627)
    ...
    p.add_argument('--late-startups',type=int,default=1);p.add_argument('--gated',action='store_true');a=p.parse_args()
    build(a.output,a.source,a.roots,a.views,a.incident_pairs,a.hot_mints,a.hot_events,a.late_startups,a.gated)
