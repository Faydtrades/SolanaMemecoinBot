...
from phase4.paper_live_observability_binding_v0_2 import (
    LOCKED_TRACKS,
    MODEL_ID,
...
def main() -> int:
    print("=" * 120)
    print("PHASE 4.5C - LIVE OBSERVABILITY RUNTIME BINDING SELF-TEST v0.2")
    print("=" * 120)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        eval_skip = SimpleNamespace(
            ...
            audit_snapshot={"source": "selftest-skip", "decimal_price": Decimal("0.0000012300"), "nested": {"ratio": Decimal("12.3400")}},
        )
        ...
        eval_trade = SimpleNamespace(
            ...
            audit_snapshot={"source": "selftest-trade", "decimal_price": Decimal("1.2500")},
        )
        ...
        decimal_json_rows = [
            str(r["audit_snapshot_json"])
            for r in conn.execute(
                "SELECT audit_snapshot_json FROM paper_strategy_evaluations ORDER BY ingest_seq"
            ).fetchall()
        ]
        ...
        checks["binding_model_id_exact"] = MODEL_ID == "P4-LIVE-OBSERVABILITY-BINDING-0002"
        checks["strategy_evaluation_runtime_mapping"] = strategy_rows == 2
        checks["decimal_audit_snapshot_json_serialized"] = (
            len(decimal_json_rows) == 2
            and '"decimal_price":"0.0000012300"' in decimal_json_rows[0]
            and '"ratio":"12.3400"' in decimal_json_rows[0]
            and '"decimal_price":"1.2500"' in decimal_json_rows[1]
        )
