...
def main(db_path: Path, out_dir: Path) -> None:
    print("PHASE 2 EMPIRICAL PARAMETER-SEARCH DESIGN HARNESS v0.1.1")
    ...
    try:
        ...
        for mint in sorted(eligible):
            ...
            metrics.append(row)
            family = price_identity_family(row.price_identity)
            if family == "SOL":
                sol_tokens += 1
            elif family == "QUOTE":
                quote_tokens += 1
        ...
        check(
            "Test 2b - SOL and quote price identities both represented and fully accounted",
            sol_tokens > 0 and quote_tokens > 0 and (sol_tokens + quote_tokens) == len(metrics),
        )
        ...
        report_path = out_dir / "phase2_parameter_search_design_report_v0_1_1.json"
        token_path = out_dir / "phase2_parameter_search_design_token_metrics_v0_1_1.csv"
        space_path = out_dir / "phase2_parameter_search_space_proposal_v0_1_1.json"
