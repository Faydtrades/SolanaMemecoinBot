...
DEFAULT_OUT_DIR = ROOT / "data" / "research" / "phase2_research_v0_3"
...
def main(db_path: Path, out_dir: Path, token_limit: int) -> None:
    print("PHASE 2 REAL-DATA RESEARCH / REPLAY HARNESS v0.3")
    ...
    check(
        "Test 3e - all probe lifecycles terminal-complete",
        all(int(probe["funnel"]["open_at_data_end"]) == 0 for probe in payload["probes"]),
    )
    check(
        "Test 3f - clock expiry offset uses strict >5m boundary",
        all(
            probe["clock"]["clock_expiry_offset_us"]["n"] == 0
            or (
                probe["clock"]["clock_expiry_offset_us"]["min"] == 300_000_001
                and probe["clock"]["clock_expiry_offset_us"]["max"] == 300_000_001
            )
    ...
    json_path = out_dir / "phase2_research_replay_v0_3.json"
    csv_path = out_dir / "phase2_research_token_metrics_v0_3.csv"
    manifest_path = out_dir / "phase2_research_manifest_v0_3.json"

    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    write_token_csv(csv_path, token_rows)
    manifest = {
        "schema_version": "P2RR-MANIFEST-0.3",
    ...
    print_metric("Pump timestamp -> observation delta (ms)", metrics["first_trade_pump_timestamp_delta_ms"])
    ...
    for probe in payload["probes"]:
        ...
        print(f"  Clock-expired           : {f['clock_expired']}")
    ...
    if open_any:
        print(
            "  * ERROR: OPEN_AT_DATA_END remains despite deterministic strategy clock. "
            "Do not use this research output until investigated."
        )
    else:
        print(
            "  * Deterministic strategy clock terminalized silent lifecycles at the entry-window deadline."
        )
    ...
    print("Deterministic strategy clock used       : YES (SCLOCK-0.1)")
