...
DEFAULT_OUT = PROJECT_ROOT / "data" / "research" / "phase2_denomination_aware_flow_v0_1_1"
...
def compatibility_params() -> FirstPullbackParameterSet:
    ...
    # parameters and candidate counts from this harness are not performance results.
...
def main(db_path: Path, out_dir: Path) -> None:
    print("PHASE 2 DENOMINATION-AWARE FLOW / STRATEGY COMPATIBILITY v0.1.1")
    print("Purpose                    : REAL-DATA REGRESSION FIX + DENOMINATION SAFETY")
    ...
    def check(label: str, ok: bool) -> None:
        print(f"{label:<86} {'PASS' if ok else 'FAIL'}")
        if not ok:
    ...
    try:
        ...
        launch_only_grouped_mints = set(grouped) - eligible_mints
        ...
        check("Test 1c - eligible universe explicitly requires >=1 tradable BUY/SELL", len(eligible_mints) > 0)
        ...
        for mint in sorted(eligible_mints):
            ...
            # Regression-fixed parity:
            # * raw SOL buy/sell/net flow must preserve the existing FeatureEngineV02 substrate;
            # * net_flow_raw in DAF intentionally covers only reserve-normalizable compatible
            #   events, so it is NOT the correct legacy raw-net comparator when an event lacks
            #   a valid reserve denominator.
            parity_ok = True
            normalized_incomplete = False
            if token_class == "SOL_ONLY":
                ...
                for event in win:
                    ...
                    for wid in ("1s", "3s", "10s", "30s", "60s", "since_t0"):
                        ...
                        # Correct comparator: all authoritative raw SOL flow.
                        if ow.net_flow_lamports != (
                            nw.sol_buy_volume_lamports - nw.sol_sell_volume_lamports
                        ):
                            parity_ok = False
                        ...
                        if nw.normalized_flow_event_count < nw.trade_count:
                            normalized_incomplete = True

                if not parity_ok:
                    sol_parity_failures += 1
                if normalized_incomplete:
                    sol_normalized_flow_incomplete_tokens += 1
            ...
            row = {
                ...
                "since_t0_compatible_net_flow_raw": since.net_flow_raw,
                "since_t0_net_flow_reserve_ppm": since.net_flow_reserve_ppm,
                "normalized_flow_events": since.normalized_flow_event_count,
                "flow_unavailable_events": since.flow_unavailable_event_count,
                "sol_legacy_raw_parity": parity_ok if token_class == "SOL_ONLY" else None,
                "sol_normalized_flow_incomplete": normalized_incomplete if token_class == "SOL_ONLY" else None,
            }
        ...
        check(
            "Test 2a - every eligible tradable token receives denomination-aware state",
            total > 0 and processed_mints == eligible_mints,
        )
        check(
            "Test 2b - all token price identities remain SOL_ONLY or QUOTE_ONLY",
            set(path_counts) <= {"SOL_ONLY", "QUOTE_ONLY"},
        )
        check("Test 2c - no flow denomination/path conflicts observed", flow_conflict_tokens == 0)
        check(
            "Test 2d - every token has a compatible reserve-normalized since_t0 flow",
            unavailable_flow_tokens == 0,
        )
        check(
            "Test 2e - strategy v1.1 mechanically evaluates every eligible token",
            compatible_tokens == total and strategy_error_tokens == 0,
        )
        check(
            "Test 2f - quote path reaches strategy without fake lamport contamination",
            quote_strategy_tokens > 0 and quote_raw_lamport_contamination == 0,
        )
        check("Test 2g - SOL path remains represented", sol_strategy_tokens > 0)

        check(
            "Test 3a - existing SOL FeatureEngineV02 RAW semantics are exactly preserved",
            sol_parity_tokens > 0 and sol_parity_failures == 0,
        )
        check(
            "Test 3b - reserve-normalized flow incompleteness is exposed, not hidden",
            sol_normalized_flow_incomplete_tokens >= 0,
        )

        summary = {
            "grouped_mints_all_event_types": len(grouped),
            "eligible_tradable_mints": len(eligible_mints),
            "launch_only_grouped_mints": len(launch_only_grouped_mints),
            ...
            "sol_legacy_raw_parity_tokens": sol_parity_tokens,
            "sol_legacy_raw_parity_failures": sol_parity_failures,
            "sol_normalized_flow_incomplete_tokens": sol_normalized_flow_incomplete_tokens,
        ...
        check(
            "Test 4a - full compatibility payload is deterministic",
            stable_sha256(summary) == stable_sha256(summary_2),
        )
        check(
            "Test 4b - common flow metric is explicitly not SOL-equivalent volume",
            summary["common_flow_feature_is_sol_equivalent_volume"] is False,
        )

        out_dir.mkdir(parents=True, exist_ok=True)
        report = {
            "schema_version": "DAF-0.1.1",
            "purpose": "DENOMINATION-AWARE FLOW + REAL-DATA REGRESSION FIX; NO PNL",
        ...
        report_path = out_dir / "phase2_denomination_aware_flow_report_v0_1_1.json"
        token_path = out_dir / "phase2_denomination_aware_flow_tokens_v0_1_1.csv"
        ...
        check(
            "Test 5a - research outputs written outside production DB",
            report_path.exists() and token_path.exists(),
        )
    finally:
        conn.close()
    ...
    print()
    print("=" * 116)
    print("DENOMINATION-AWARE FLOW / STRATEGY COMPATIBILITY v0.1.1 — NO PNL")
    print("=" * 116)
    print(f"Grouped mints, all event types             : {summary['grouped_mints_all_event_types']}")
    print(f"Eligible tradable mints                    : {summary['eligible_tradable_mints']}")
    print(f"Launch-only grouped mints                  : {summary['launch_only_grouped_mints']}")
    ...
    print(f"SOL legacy RAW parity failures            : {summary['sol_legacy_raw_parity_failures']}/{summary['sol_legacy_raw_parity_tokens']}")
    print(f"SOL tokens with some unnormalizable flow  : {summary['sol_normalized_flow_incomplete_tokens']}")
    ...
    print()
    print("IMPORTANT")
    print(" * Raw SOL parity is checked against ALL authoritative SOL trade amounts.")
    print(" * net_flow_raw / net_flow_reserve_ppm cover only events with a valid reserve denominator.")
    print(" * An unavailable reserve denominator therefore does NOT erase raw SOL flow; it only")
    print("   prevents that event from contributing to the reserve-normalized common-flow feature.")
    print(" * net_flow_reserve_ppm is NOT SOL-equivalent volume and does not convert quote units to SOL.")
    print(" * Candidate/state counts use mechanical compatibility values only; they are NOT strategy results.")
