...
def main():
    ...
    result=run_locked_entry_readiness(
        freeze_db=freeze/"phase3_development_dataset_v0_2.sqlite3",
        freeze_manifest=freeze/"phase3_development_dataset_manifest_v0_2.json",
        coverage_snapshot=freeze/"collector_coverage_DEV-FREEZE-0002_v0_2.json",
        selection_path=p3/"EXP-0005/EXP-0005_D_selection_v0_1.json",
    )
    out=p3/"EXP-0007"
    rp=out/"DEV-FREEZE-0002_locked_entry_readiness_report_v0_2.json"
    cp=out/"DEV-FREEZE-0002_locked_entry_readiness_by_regime_v0_2.csv"
    atomic(rp,result)
    rows=[]
    ...
    for r in result["regimes"]:
        ...
        for s in statuses:row[f"status_{s}"]=r["path_status_counts"].get(s,0)
        rows.append(row)
    with cp.open("w",encoding="utf-8",newline="") as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0].keys()));w.writeheader();w.writerows(rows)

    print("EXP-0007 DEV-FREEZE-0002 LOCKED-ENTRY READINESS v0.1.2")
    print("="*74)
    for r in rows:
        print(f"{r['role']:8s} candidates={r['candidate_count']:4d} clean5m={r['clean_5m_count']:4d} rate={100*r['clean_5m_rate']:6.2f}%")
    rd=result["readiness"];print("-"*74)
