...
SCHEMA_VERSION = "phase4_paper_exit_lifecycle_bridge_v0.2"
ENGINE_VERSION = "PaperExitLifecycleBridgeV02"
...
class PaperExitLifecycleBridgeV02:
    """Bind persisted FILLED entry state to locked Phase-4 exit intent state.

    v0.2 additionally supports mixed OPEN / EXIT_PENDING / CLOSED lifecycle
    states after the three locked tracks begin exiting at different times.
    ...
    def bind_signal(self, signal_key: str) -> tuple[ExitPositionRef, ...]:
        ...
        # v0.2: once parallel exits begin, one track may already be CLOSED while
        # another remains OPEN/trailing or EXIT_PENDING. list_open_positions()
        # intentionally excludes CLOSED state, so it cannot be the rebinding
        # source after the first close. Reconstruct all three lifecycle positions
        # by persisted signal lineage instead.
        position_ids = [
        ...
        for position_id in position_ids:
            position = self.lifecycle.get_position(position_id)
            if position is None:
                raise ExitLifecycleBindingError(
                    f"signal {signal_key} references missing lifecycle position {position_id}"
                )
        ...
        if len(positions) != len(LOCKED_TRACK_ORDER) or set(by_track) != set(LOCKED_TRACK_ORDER):
            raise ExitLifecycleBindingError(
                f"signal {signal_key} must have exactly {LOCKED_TRACK_ORDER} lifecycle positions; "
    ...
    def apply_evaluations(
        ...
        for evaluation in evaluations:
            ...
            if intent is not None:
                ...
                # v0.2 terminal replay semantics: after a causal exit executor
                # has CLOSED one track, the orchestrator still returns its
                # immutable persisted ExitIntent on later parallel observations.
                # That must not block the still-live sibling tracks. CLOSED is
                # therefore accepted as terminal and never mutated/reopened.
                if position.state is PositionState.CLOSED:
                    if not position.state_reason.startswith("PAPER_EXIT_FILLED:"):
                        raise ExitLifecycleBindingError(
                            "CLOSED position does not carry an exit-fill terminal reason"
                        )
                else:
