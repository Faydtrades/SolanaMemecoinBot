...
SCHEMA_VERSION = "phase3_outcome_replay_v0.1.1"
...
class OutcomeReplayV011:
    ...
    - A return is considered OBSERVABLE only when collector coverage is
      COMPLETE for the whole candidate->horizon interval.
    - If coverage is GAP/UNKNOWN, an available mark can still be recorded for
      audit, but status remains non-observable.
    - MFE/MAE are observed extrema through 5m only. They are None when no fresh,
      same-identity future price exists. extrema_complete_5m requires COMPLETE
      coverage, at least one fresh future price, and no price-identity mismatch
      or GAP_RECOVERY provenance.
    """
