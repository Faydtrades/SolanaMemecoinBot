...
class CandidateExtractionSummary:
    ...
    @property
    def accounted_run_count(self) -> int:
        return (
            self.invalidated_count
            + self.expired_count
            + self.rejected_count
            + self.trade_count
            + self.candidate_state_count
        )
...
def extract_candidates(
    ...
    summary = CandidateExtractionSummary(
        ...
        rejected_count=sum(r.current_state == FirstPullbackState.REJECT for r in runs.values()),
        trade_count=sum(r.current_state == FirstPullbackState.TRADE for r in runs.values()),
