...
def price_identity_family(label: str) -> str:
    """Map the explicit price-identity label to a coarse research family.

    SOL price identities are intentionally labelled SOL_NATIVE by the
    quote-aware price layer. Quote identities are labelled QUOTE:<mint>.
    """
    text = str(label)
    if text == "SOL_NATIVE":
        return "SOL"
