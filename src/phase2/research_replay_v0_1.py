...
RESEARCH_SCHEMA_VERSION = "P2RR-0.1"
    ...
    *,
    ...
    for mint in cohort_mints:
        ...
        row: dict[str, object] = {
            ...
            "first_trade_observation_delay_ms": (
...
def run_strategy_probe(
    ...
    for mint in cohort_mints:
        ...
        for event in token_events:
            ...
            last_age_ms = state.identity.age_ms
            if evaluation.transition_occurred:
                ...
                if state_name in stage_age_ms and state.identity.age_ms is not None:
                    stage_age_ms[state_name].append(int(state.identity.age_ms))
