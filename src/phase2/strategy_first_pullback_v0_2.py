...
from .denomination_aware_flow_v0_1 import DenominationAwareMarketState, FlowCompatibility
...
class FirstPullbackStrategyV02:
    """Deterministic First Pullback v1 state machine.

    This module interprets DenominationAwareMarketState only; it does not read the database, wall clock,
    ...
    Parameter semantics used by v0.1:
      ...
      buyer_response_parameters:
        ...
        min_buys
        min_net_flow_reserve_ppm       (dimensionless reserve-relative flow proxy)
      reclaim_parameters:
    ...
    `min_net_flow_reserve_ppm` is NOT a SOL-equivalent volume threshold.  It is
    a dimensionless reserve-relative pressure feature whose parameter range must
    be researched separately.  SOL and quote raw volumes remain separate.
    """
    ...
    def validate_parameters(cls, params: FirstPullbackParameterSet) -> None:
        ...
        cls._require_int(params.buyer_response_parameters, "min_net_flow_reserve_ppm")
    ...
    @classmethod
    def start_run(
        cls, market_state: DenominationAwareMarketState, params: FirstPullbackParameterSet
    ) -> StrategyRunState:
    ...
    @staticmethod
    def _point(state: DenominationAwareMarketState) -> MarketPoint | None:
    ...
    @staticmethod
    def _audit_snapshot(state: DenominationAwareMarketState, response_window_id: str) -> dict[str, object]:
        ...
        return {
            ...
            "response_window_net_flow_reserve_ppm": window.net_flow_reserve_ppm if window else None,
            "response_window_flow_identity": window.flow_identity.label if window else None,
            "response_window_flow_compatibility": window.flow_compatibility.value if window else None,
        }

    @classmethod
    def _evaluation(
        cls,
        state: DenominationAwareMarketState,
    ...
    def _transition(
        ...
        if new_state in FirstPullbackStrategyV02.TERMINAL_STATES:
            run.finished = True
    ...
    def evaluate(
        ...
        if run.current_state == FirstPullbackState.WAITING_FOR_RESPONSE:
            ...
            min_flow = cls._require_int(
                params.buyer_response_parameters, "min_net_flow_reserve_ppm"
            )
            ...
            checks = {
                ...
                "buyer_response_flow_compatible": window.flow_compatibility == FlowCompatibility.COMPATIBLE,
                "buyer_response_net_flow": window.net_flow_reserve_ppm >= min_flow,
            }
            passed = tuple(k for k, v in checks.items() if v)
            failed = tuple(k for k, v in checks.items() if not v)
            if all(checks.values()):
